...
export const acceptTicket = id => async dispatch => {
	try {
		...
		console.log(resp);
		toast.success("Ticket succesfully approved",{autoClose:2000});
		dispatch({
		...
		console.log(err);
		toast.error("Error occured during action",{autoClose:2000});
		dispatch({
...
export const rejectTicket = id => async dispatch => {
	try {
		...
		console.log(resp);
		toast.success("Ticket succesfully rejected",{autoClose:2000});
		dispatch({
			type: ACTIONS.ACTION_ON_TICKET,
			payload: resp.data,
		});
...
export const holdTicket = id => async dispatch => {
	try {
		...
		console.log(resp);
		toast.success("Ticket put on hold",{autoClose:2000});
		dispatch({
			type: ACTIONS.ACTION_ON_TICKET,
			payload: resp.data,
		});
...
export const reactivateTicket = id => async dispatch => {
	try {
		...
		console.log(resp);
		toast.success("Ticket reactivated",{autoClose:2000});
		dispatch({
			type: ACTIONS.ACTION_ON_TICKET,
			payload: resp.data,
		});
